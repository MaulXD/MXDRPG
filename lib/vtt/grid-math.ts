/** Célula do grid (q = coluna, r = linha). Coordenadas axiais (`Axial`) na API. */
...
export function cellsInRange(origin: Axial, range: number): Axial[] {
  const result: Axial[] = [];
...
export const GRID_DIRECTIONS: Axial[] = [
  { q: 1, r: 0 },
  { q: 1, r: 1 },
  { q: 0, r: 1 },
  { q: -1, r: 1 },
...
export const GRID_DIRECTION_COUNT = GRID_DIRECTIONS.length;

export function cellNeighbors(a: Axial): Axial[] {
  return GRID_DIRECTIONS.map((d) => ({ q: a.q + d.q, r: a.r + d.r }));
}

export function cellDirection(from: Axial, to: Axial): number | null {
  const dq = to.q - from.q;
  const dr = to.r - from.r;
  const idx = GRID_DIRECTIONS.findIndex((d) => d.q === dq && d.r === dr);
  return idx >= 0 ? idx : null;
}

/** Meio lado da célula (raio do quadrado desenhado). */
export function cellDrawRadius(cellSize: number): number {
  return cellSize / 2;
}

export function cellCorners(cx: number, cy: number, halfSize: number): { x: number; y: number }[] {
  const hs = halfSize;
  return [
    { x: cx - hs, y: cy - hs },
    { x: cx + hs, y: cy - hs },
...
export function inSquareGrid(cell: Axial, gridRadius: number): boolean {
  return Math.abs(cell.q) <= gridRadius && Math.abs(cell.r) <= gridRadius;
}
