...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const itemRequest = await getInventoryItemRequest(requestId);
  ...
  const adventure = await getAdventure(itemRequest.adventureId);
  ...
  const updated = await resolveInventoryItemRequestByGm(requestId, body.action, session.user.id);
  if (!updated) {
